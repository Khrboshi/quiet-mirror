...
> **Before payments go live:** Vercel's Hobby plan is restricted to personal, non-commercial use, and their fair-use terms define commercial usage to include any deployment that requests or processes payment from visitors. A **Pro upgrade is required before enabling Dodo billing**, not after. Supabase is likewise on the Free plan, which has no automated backups and no point-in-time recovery — see `docs/BETA_READINESS.md`.
