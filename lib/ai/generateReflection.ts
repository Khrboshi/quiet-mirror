...
// Havenly V24 - reflection quality hardening
// Fixes included:
// 1) Stronger vivid-anchor extraction and quote handling
// 2) Generic summary opener rejection remains enforced
// 3) Premium output always includes "Deeper direction:" and "Script line:"
// 4) Short FITNESS negative entries no longer default to "pride"
// 5) Safer normalization and fallback behavior
...
const DOMAIN_DEFAULTS: Record<Domain, DomainDefaults> = {
  ...
  WORK: {
    ...
    corepattern:
      "The gap between what you contributed and what was recognized is where the feeling lives.",
    themes: ["recognition", "visibility", "respect", "professional worth"],
    emotions: ["frustration", "hurt", "self-doubt", "anger"],
    nextStepFree:
      "Option A: Write down what you wish had happened instead - specifically. Option B: Name the one person at work who actually sees your contribution.",
    nextStepPremium:
    ...
    shortNextStep:
      "Option A: Write one sentence about what you actually needed from that situation. Option B: Name whether this is about one incident or a pattern.",
    questions: [
  ...
  RELATIONSHIP: {
    ...
    shortNextStep:
      "Option A: Write one honest sentence about what you needed. Option B: Notice whether this is about this moment or a longer pattern.",
    questions: [
  ...
  MONEY: {
    ...
    nextStepFree:
      "Option A: Write down the specific financial fact that feels most urgent - just the fact, not the spiral. Option B: Name one action that would reduce the pressure even slightly.",
    nextStepPremium:
    ...
    shortNextStep:
      'Option A: Is this about a specific number, or the feeling behind it? Write one sentence. Option B: What would feel like enough safety right now - not solved, just safer?',
    questions: [
...
function isCarryingLineBanned(line: string): boolean {
  return BANNED_SUMMARY_PATTERNS.some((re) => re.test(line.toLowerCase()));
}
...
function ensurePremiumSummary(
  ...
  if (!s.includes("Deeper direction:")) {
    ...
    const domainDD: Record<Domain, string> = {
      ...
      GENERAL:
        "Deeper direction: The clearest fact in what you wrote is the thing most worth staying with. Name it in one plain sentence before deciding what to do next.",
    };
...
function isNegativeFitnessShortEntry(text: string): boolean {
  const s = text.toLowerCase();
  return (
    /\b(skip|skipped|didn't go|did not go|couldn't go|could not go|missed|blew off|avoided|stayed home|did nothing|didnt go)\b/i.test(
      s
    ) &&
    /\b(gym|workout|training|run|running|exercise|cardio|lifting)\b/i.test(s)
  );
}
...
function normalizeReflection(
  ...
  let emotionFallback = defaults.emotions;
  if (domain === "FITNESS" && short && isNegativeFitnessShortEntry(entryBody)) {
    emotionFallback = ["disappointment", "frustration", "guilt", "tiredness"];
  }

  let emotions = topUpUnique(cleanArr(r?.emotions, 6), emotionFallback, minArr, 6);
  if (domain === "FITNESS" && short && isNegativeFitnessShortEntry(entryBody)) {
    emotions = emotions.filter((e) => normalizeForMatch(e) !== "pride");
    emotions = topUpUnique(emotions, emotionFallback, minArr, 6);
  }
...
export async function generateReflectionFromEntry(input: Input): Promise<Reflection> {
  ...
  const vividAnchor = selectVividAnchor(anchors);
  const domainAnchor = domainAnchorRe ? anchors.find((a) => domainAnchorRe.test(a)) ?? null : null;
  const vividScore = vividAnchor ? scoreAnchorVividness(vividAnchor) : 0;

  const bestAnchor =
    vividScore >= 2 ? vividAnchor ?? domainAnchor ?? anchors[0] ?? "" : domainAnchor ?? vividAnchor ?? anchors[0] ?? "";
  ...
  return normalizeReflection(
    {
      ...
      emotions:
        domain === "FITNESS" && short && isNegativeFitnessShortEntry(entryBody)
          ? ["disappointment", "frustration", "guilt", "tiredness"]
          : defaults.emotions,
