...
- [ ] Confirm backup/recovery expectations for database and user content — **checked 31 Aug 2026: there are none.** Supabase Free has no automated backups and no point-in-time recovery. The only copy of all user journal content is one unbacked database. Options: upgrade Supabase to Pro (7-day daily backups), or build the self-serve export route (`SAAS_ANALYSIS.md` idea #3) and run it on a schedule. **Do not invite outside testers until one of these exists**
